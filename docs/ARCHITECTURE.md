...
**Production URL:** `https://stocks.starbhai.com` (branded canonical subdomain), with
`https://stock-platform-gamma.vercel.app` retained as the stable legacy Vercel alias. The latest
`stock-platform-<hash>-…vercel.app` deployment URL rotates per push. **`starbhai.com` is NOT this
platform** — it 301s to a separate WordPress site. Link the `stocks.starbhai.com` subdomain for the
platform and keep the `-gamma` alias available during rollout.
...
```
 ...
 3. cors()                             allowlist: stocks.starbhai.com, -gamma, localhost:3000/4011, preview regex
