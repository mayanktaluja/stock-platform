...
function capReason({ tradeRupees, roomPosition, roomSector, remaining }) {
  if (remaining < MIN_FUNDED_TRADE_INR) return `leftover cash below INR ${MIN_FUNDED_TRADE_INR.toLocaleString("en-IN")} minimum`;
  if (roomPosition < MIN_FUNDED_TRADE_INR) return "single-name 8% cap leaves no minimum trade room";
...
export function buildPortfolioConstructionPlan({
  ...
  for (const c of eligibleAddCandidates) {
    ...
    const roomPosition = Math.max(0, basePostTradeValue * (MAX_POSITION_WEIGHT_PCT / 100) - currentValue);
    ...
    if (tradeRupees >= MIN_FUNDED_TRADE_INR) {
      ...
      const fundedTrade = {
        ...
        reason: `${c.valuation_band} with ${c.upside_pct?.toFixed?.(1) ?? c.upside_pct}% FV upside, v4 ${c.v4_score ?? "n/a"}, within post-trade caps`,
        v4_score: c.v4_score,
        upside_pct: c.upside_pct,
        valuation_band: c.valuation_band,
        valuation_confidence: c.valuation_confidence,
      ...
      c.unfundedReasons.push(capReason({ tradeRupees, roomPosition, roomSector, remaining }));
    }
  }
  ...
  const fundedSells = [];
  for (const h of scoredHoldings) {
    if (!h?.swsCovered || !ALL_REDUCTION_ACTIONS.has(h.action)) continue;
