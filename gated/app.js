...
// ──────────────────────────────────────────────────────────────────────────
// Credibility ribbon — surfaces the SEBI-RA-grade top-5-per-section forward
// returns vs Nifty 50 and Nifty 500 at the top of the SWS Picks tab. Pulls
  ...
  try {
    ...
    if (subtitleEl) {
      ...
      subtitleEl.textContent =
        `Last ${data.horizonDays || 30} days · ${data.cohortsInWindow || 0} cohorts · ` +
        `${obs} ticker-observations · vs Nifty 50` +
        (n500ok ? " and Nifty 500" : " (Nifty 500 unavailable)");
    }
    ribbonsEl.innerHTML = data.sections.map(renderCredibilityRibbon).join("");
    renderCredibilityChart(chartEl, data.chart);
  } catch (err) {
    console.warn("[CREDIBILITY] load failed:", err && err.message);
    section.style.display = "none";
  }
}

function renderCredibilityRibbon(s) {
  if (!s.mature) {
    return `
      <div class="credibility-ribbon warming" data-section="${s.slug}">
        <div class="credibility-ribbon-label">${escapeHtml(s.label)}</div>
        <div class="credibility-ribbon-value warming-text">Warming up · ${s.observationCount || 0} picks</div>
        <div class="credibility-ribbon-benchmarks">${s.cohortCount || 0} cohort${(s.cohortCount || 0) === 1 ? "" : "s"}</div>
      </div>`;
  }
  const fmt = (v) => v == null ? "—" : `${v >= 0 ? "+" : ""}${v.toFixed(2)}%`;
  const fmtAlpha = (v) => v == null ? "—" : `${v >= 0 ? "+" : ""}${v.toFixed(2)}pp`;
  const valueClass = s.avgReturnPct == null ? "" : (s.avgReturnPct >= 0 ? "pos" : "neg");
  const alpha50Class = s.alphaN50Pct == null ? "" : (s.alphaN50Pct >= 0 ? "pos" : "neg");
  const alpha500Class = s.alphaN500Pct == null ? "" : (s.alphaN500Pct >= 0 ? "pos" : "neg");
  const picks = (s.topPicksLatest || []).slice(0, 5).join(" · ");
  return `
    <div class="credibility-ribbon" data-section="${s.slug}" title="${s.cohortCount} cohorts · ${s.observationCount} picks · ${s.winRatePct ?? '—'}% beat Nifty 50">
      <div class="credibility-ribbon-label">${escapeHtml(s.label)}</div>
      <div class="credibility-ribbon-value ${valueClass}">${fmt(s.avgReturnPct)}</div>
      <div class="credibility-ribbon-benchmarks">
        <div>vs Nifty 50: ${fmt(s.niftyReturnPct)} <span class="alpha ${alpha50Class}">${fmtAlpha(s.alphaN50Pct)}</span></div>
        <div>vs Nifty 500: ${fmt(s.n500ReturnPct)} <span class="alpha ${alpha500Class}">${fmtAlpha(s.alphaN500Pct)}</span></div>
      </div>
      <div class="credibility-ribbon-picks">${escapeHtml(picks)}</div>
    </div>`;
}

// Tri-line SVG chart — Top-5 equal-weight portfolio (gold) vs Nifty 50 (gray)
// vs Nifty 500 (dim blue) over the reference window. Reuses the layout
// approach from renderTrackChart but plots daily resolution instead of
// monthly buckets.
function renderCredibilityChart(container, chart) {
  if (!container) return;
  if (!chart || !Array.isArray(chart.dates) || chart.dates.length < 2) {
    container.innerHTML = `<div class="credibility-chart-empty">Chart appears once we have at least two trading days of post-snapshot data.</div>`;
    return;
  }
  const dates = chart.dates;
  const portfolio = chart.topFivePct || [];
  const n50 = chart.nifty50Pct || [];
  const n500 = chart.nifty500Pct || [];

  const W = 980, H = 280;
  const PAD = { top: 18, right: 24, bottom: 40, left: 52 };
  const plotW = W - PAD.left - PAD.right;
  const plotH = H - PAD.top - PAD.bottom;

  const allVals = [...portfolio, ...n50, ...n500].filter((v) => Number.isFinite(v));
  if (allVals.length < 2) {
    container.innerHTML = `<div class="credibility-chart-empty">Not enough price data to plot.</div>`;
    return;
  }
  const rawMin = Math.min(...allVals);
  const rawMax = Math.max(...allVals);
  ...
  const xAt = (i) => PAD.left + (dates.length === 1 ? plotW / 2 : (i / (dates.length - 1)) * plotW);
  const yAt = (v) => PAD.top + plotH - ((v - yMin) / yRange) * plotH;

  const tickCount = 5;
  const ticks = [];
  for (let i = 0; i <= tickCount; i++) ticks.push(yMin + (yRange * i) / tickCount);
  ...
  const colorPicks = "#E0B060";
  const colorN50 = "rgba(237, 237, 237, 0.45)";
  const colorN500 = "rgba(96, 165, 250, 0.55)";

  const finalVal = (s) => {
    for (let i = s.length - 1; i >= 0; i--) if (Number.isFinite(s[i])) return s[i];
    return null;
  };
  const portfolioFinal = finalVal(portfolio);
  const n50Final = finalVal(n50);
  const n500Final = finalVal(n500);
  ...
  const gridLines = ticks.map((v) => {
    ...
    return `<line x1="${PAD.left}" y1="${y}" x2="${W - PAD.right}" y2="${y}"
              stroke="${isZero ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.05)'}"
              stroke-width="1" stroke-dasharray="${isZero ? '0' : '2 3'}" />
            <text x="${PAD.left - 8}" y="${y + 4}" text-anchor="end">${v >= 0 ? "+" : ""}${v.toFixed(1)}%</text>`;
  }).join("");

  const labelEvery = Math.max(1, Math.ceil(dates.length / 8));
  const dateLabels = dates.map((d, i) => {
    if (i !== 0 && i !== dates.length - 1 && i % labelEvery !== 0) return "";
    return new Date(d).toLocaleDateString("en-IN", { day: "2-digit", month: "short" });
  });
  const xTicks = dateLabels.map((label, i) => {
    if (!label) return "";
    return `<text x="${xAt(i)}" y="${H - PAD.bottom + 16}" text-anchor="middle">${label}</text>`;
  }).join("");

  const tickers = (chart.tickers || []).join(" · ");
  const sectionLabel = chart.sectionLabel || "Top 5";
  ...
  container.innerHTML = `
    <div class="track-chart-wrap">
      <div class="track-chart-legend">
        <span class="track-chart-legend-item">
          <span class="track-chart-legend-swatch" style="background:${colorPicks};"></span>
          ${escapeHtml(sectionLabel)} — Top 5 portfolio <strong style="color:${portfolioFinal >= 0 ? '#86efac' : '#fca5a5'};margin-left:4px;">${fmtPct(portfolioFinal)}</strong>
        </span>
        <span class="track-chart-legend-item">
          <span class="track-chart-legend-swatch" style="background:${colorN50};"></span>
          Nifty 50 <strong style="color:var(--text-secondary);margin-left:4px;">${fmtPct(n50Final)}</strong>
        </span>
        <span class="track-chart-legend-item">
          <span class="track-chart-legend-swatch" style="background:${colorN500};"></span>
          Nifty 500 <strong style="color:var(--text-secondary);margin-left:4px;">${fmtPct(n500Final)}</strong>
        </span>
      </div>
      <svg class="track-chart" viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" role="img"
           aria-label="Top 5 picks vs Nifty 50 and Nifty 500 — cumulative return">
        ${gridLines}
        ${xTicks}
        <path d="${pathFor(n500)}" fill="none" stroke="${colorN500}" stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round" />
        <path d="${pathFor(n50)}"  fill="none" stroke="${colorN50}"  stroke-width="1.5" stroke-linejoin="round" stroke-linecap="round" />
        <path d="${pathFor(portfolio)}" fill="none" stroke="${colorPicks}" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" />
      </svg>
      <div class="track-chart-note">
        Equal-weighted Top-5 from <strong>${escapeHtml(sectionLabel)}</strong> picked on
        <strong>${escapeHtml(chart.referenceCohortDate || "—")}</strong>${tickers ? ` (${escapeHtml(tickers)})` : ""}, held forward and
        compared with Nifty 50 and Nifty 500 over the same window. Daily closes.
      </div>
    </div>
  `;
}
...
async function loadPicks() {
  ...
  // Fire-and-forget — credibility ribbon hydrates independently and won't
  // block the picks accordion below.
  loadPicksCredibility();
