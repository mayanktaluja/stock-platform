...
function renderNewsPage(digest, heatmap, macroRegime, catalysts, discovery, fiiDii, indexIntraday) {
  ...
  // breadth → review queue (collapsed). market-intelligence.spec.mjs chains
  // macro < digest < catalysts < heatmap < radar.
...
function syncScanStatusPollers() {
  ...
  // Any future polling tab should be wired here rather than via a local
  // visibilitychange listener: this function is already called from the end of
  // switchTab AND from the single visibilitychange handler below, so a
  // backgrounded tab stops polling for free.
}
