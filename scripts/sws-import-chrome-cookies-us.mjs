...
import { profileDirForShard, SHARED_FINGERPRINT } from "./sws-config-us.mjs";
...
async function main() {
  ...
  if (isChromeRunning()) {
    // US fork: do NOT refuse while Chrome is open. readSimplyWallCookies() copies
    // the Cookies DB (+ WAL/SHM) to /tmp before reading, which is a consistent
    // snapshot even mid-session — so we keep the user's browser + Chrome MCP intact.
    console.warn("⚠ Chrome is running — reading cookies from a /tmp snapshot copy (WAL-safe).");
  }
