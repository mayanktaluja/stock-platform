#!/usr/bin/env bash
# Korea SWS pipeline status — one-shot snapshot of the KR picks pipeline (data/sws-kr/).
# The Korea analog of scripts/sws-status.sh (India) and sws-status-us.sh (US). Read-only.
# Auto-detects the live KR data dir (the repo's own data/sws-kr, or an active git
# worktree's), so it works whether the scrape runs in-repo or in a worktree. The KR
# region pipeline lives on origin/main (not the cron-owned auto-refresh branch), so it
# normally runs from a worktree.
# Override detection with: SWS_KR_DIR=/abs/path/data/sws-kr
set -o pipefail
...
DETECT_NOTE=""
if [ -z "$DATA_DIR" ]; then
  # Tier 1: if a KR scrape is LIVE, report ITS worktree (resolve the proc's cwd).
  # Beats mtime so a watch never gets stolen by another worktree's fresh write.
  for pid in $(ps -A -o pid=,command= | grep -E 'sws-api-scrape-region\.mjs.*--region[ ]+kr' | grep -v grep | awk '{print $1}'); do
    cwd=$(lsof -a -p "$pid" -d cwd -Fn 2>/dev/null | sed -n 's/^n//p' | head -1)
    if [ -n "$cwd" ] && [ -d "$cwd/data/sws-kr" ]; then DATA_DIR="$cwd/data/sws-kr"; DETECT_NOTE="  [live scrape pid $pid]"; break; fi
  done
  ...
  if [ -z "$DATA_DIR" ]; then
    ...
    for d in "$REPO/data/sws-kr" "$REPO"/.claude/worktrees/*/data/sws-kr; do
      [ -d "$d" ] || continue
      cands=$((cands + 1))
      t=$(stat -f %m "$d"/progress-api-*.json "$d"/last-refresh.json "$d"/refresh-kr-shard-*.log "$d"/picks-latest.json 2>/dev/null | sort -rn | head -1)
      [ -z "$t" ] && t=0
      if [ "$t" -gt "$bestt" ]; then bestt="$t"; best="$d"; fi
    done
    DATA_DIR="$best"
    [ "$cands" -gt 1 ] && DETECT_NOTE="  [!] $cands candidate dirs, no live scrape -- picked most-recent; pin with SWS_KR_DIR= if this is the wrong run"
  fi
fi

echo "============================================================"
echo "  Korea SWS pipeline status — $(date '+%Y-%m-%d %H:%M:%S %Z')"
echo "============================================================"
if [ -z "$DATA_DIR" ] || [ ! -d "$DATA_DIR" ]; then
  echo "  x No data/sws-kr found (repo or worktrees). Has a KR scrape run yet?"
  exit 0
fi
...
echo ""
echo "-- live KR scrape processes --"
ps -A -o pid,etime,command | grep -E 'sws-api-scrape-region\.mjs.*--region[ ]+kr|sws-refresh-region\.sh[ ]+kr' | grep -v grep || echo "  (none -- KR scrape not running)"

echo ""
echo "-- co-run safety (ONE shared SWS account -- India/US/Taiwan must NOT scrape concurrently) --"
SIB=""
...
T=$(ps -A -o command= | grep -E 'sws-api-scrape-region\.mjs.*--region[ ]+tw|sws-refresh-region\.sh[ ]+tw' | grep -vE 'grep|gh pr|pr create|--body|sws-status' | head -1)
[ -n "$I" ] && { echo "  ! India scrape RUNNING -- a KR refresh will refuse (exit 5):"; echo "      $I"; SIB=1; }
[ -n "$U" ] && { echo "  ! US scrape RUNNING -- a KR refresh will refuse (exit 5):"; echo "      $U"; SIB=1; }
[ -n "$T" ] && { echo "  ! Taiwan scrape RUNNING -- a KR refresh will refuse (exit 5):"; echo "      $T"; SIB=1; }
[ -z "$SIB" ] && echo "  ok: no India / US / Taiwan scrape running"
...
TOT=0
for s in 1 2 3; do
  l="$DATA_DIR/refresh-kr-shard-$s.log"
  if [ -f "$l" ]; then
...
echo ""
echo "-- picks-latest.json (what the Korea Picks tab serves) --"
...
if [ -f "$DATA_DIR/panic-stop.flag" ]; then echo ""; echo "  ! PANIC FLAG SET -- KR refresh refuses until removed: $DATA_DIR/panic-stop.flag"; fi

echo ""
echo "-- logs (tail -f to watch) --"
echo "  orchestrator: /tmp/sws-kr-refresh.log"
echo "  $DATA_DIR/refresh-kr-shard-{1,2,3}.log"
