...
# Set when the degraded data-only auto-ship was attempted and failed. Global (not
# local) because `set -u` is on and it must survive to the final exit below.
DATA_SHIP_FAILED=0
...
# Date/branch labels — computed before the health gate so every downstream
# path (PASS publish, sanity-gate data-only PR, health-gate data-only PR)
# can use them.
DATE=$(date +%Y-%m-%d)
...
Auto-generated by scripts/sws-nightly.sh." \
    ...
    if [ "${ship_rc}" -ne 0 ]; then
      # Previously this path swallowed every failure (sws-auto-ship.sh returned 0
      # unconditionally), so a data-only ship could stall with no mail, no
      # non-zero exit, and no signal at all — just a stale dashboard.
      DATA_SHIP_FAILED=1
      DATA_PR_NOTE="🚨 Data-only auto-ship FAILED for ${data_changed} file(s) (rc=${ship_rc}) — the data did NOT reach prod. See the [auto-ship] lines above."
    else
...
if [ "${HEALTH_GATE_RC}" -ne 0 ]; then
  # Blocking SWS picks must not also strand the independent fresh inputs
  # (fundamentals, catalysts, F&O, earnings) on this machine — during the
  # 2026-07-09 → 11 surveillance outage that turned one stale input into a
  # whole-platform stale banner. Ship whatever non-SWS data did refresh.
  ship_nightly_data_only "Snapshot health gate failed"
...
if printf '%s' "${HEALTH_GATE_OUT}" | grep -q "DEGRADED "; then
  send_mail "⚠️ SWS nightly — publishing with a degraded snapshot input" \
"The health gate passed in DEGRADED mode: one or more inputs are past their
freshness target but inside the hard-fail band, so the run continues and
production keeps serving the last-good copy of that input. Fix the refresh
before it crosses the hard-fail age and starts blocking publishes.
...
Inspect:
  cd ${REPO_DIR}
  node scripts/check-snapshot-health.mjs --strict --critical-only
  node scripts/refresh-surveillance.mjs --strict"
fi
...
# ── PR create + reconcile (extracted verbatim by test/swsNightlyPrReconcile.test.sh) ──
# Root cause of the 2026-08-11 orphaned PR. GitHub's GraphQL endpoint answered
# `gh pr create` with `HTTP 502: 502 Bad Gateway` AFTER the createPullRequest
# mutation had already committed server-side. The PR existed as #1272, but gh
# reported rc=1, this block took the error path, mailed the operator and exited
# 8 — so the auto-merge step below never ran at all. The PR then sat open with
# every required check green until a human merged it 4h25m later, and prod
# served day-old data the whole time. The identical 502-after-commit was seen
# independently on `gh pr merge` the same night.
#
# The lesson: a non-zero gh rc means "outcome unknown", never "nothing
# happened". Reconcile against the head branch, which is authoritative server
# state, before treating a create as failed. Retries are safe because the
# reconcile lookup runs first on every pass, so a duplicate PR cannot be opened.
#
# gh pr create also writes warnings ("Warning: 5454 uncommitted changes") to
# stderr. Capture stdout+stderr for diagnostics, but extract ONLY the
# pull-request URL for the ref passed to gh pr merge — otherwise the warning
# text gets concatenated with the URL and the subsequent merge fails with
# "invalid qualified head ref format".
pr_url_for_branch() {
  gh pr list --head "$1" --state open --json url --jq '.[0].url // empty' 2>/dev/null
}

extract_pr_url() {
  printf '%s\n' "$1" | grep -oE 'https://github\.com/[^[:space:]]+/pull/[0-9]+' | tail -1
}

PR_CREATE_ATTEMPTS="${SWS_NIGHTLY_PR_CREATE_ATTEMPTS:-3}"
PR_RETRY_BASE_SECONDS="${SWS_NIGHTLY_PR_RETRY_BASE_SECONDS:-15}"
PR_URL=""
PR_OUTPUT=""
PR_RC=0
pr_attempt=1
while [ "${pr_attempt}" -le "${PR_CREATE_ATTEMPTS}" ]; do
  # Adopt a PR that a previous attempt (or a lost-response 502) already opened
  # before spending another create call on the same branch.
  PR_URL="$(pr_url_for_branch "${BRANCH}")"
  if [ -n "${PR_URL}" ]; then
    [ "${pr_attempt}" -gt 1 ] && echo "[nightly] reconciled ${BRANCH} to its existing open PR: ${PR_URL}"
    break
  fi
  ...
  # rc!=0, or a rc=0 whose URL we failed to parse: ask the server what exists.
  PR_URL="$(pr_url_for_branch "${BRANCH}")"
  if [ -n "${PR_URL}" ]; then
    echo "[nightly] gh pr create reported rc=${PR_RC} but the mutation landed — reconciled to ${PR_URL}"
    break
  fi

  if [ "${pr_attempt}" -lt "${PR_CREATE_ATTEMPTS}" ]; then
    echo "[nightly] gh pr create attempt ${pr_attempt}/${PR_CREATE_ATTEMPTS} failed (rc=${PR_RC}); no PR on ${BRANCH} — retrying in $((pr_attempt * PR_RETRY_BASE_SECONDS))s"
    sleep $((pr_attempt * PR_RETRY_BASE_SECONDS))
  fi
  pr_attempt=$((pr_attempt + 1))
done

if [ -z "${PR_URL}" ]; then
  echo "[nightly] gh pr create failed after ${PR_CREATE_ATTEMPTS} attempt(s): ${PR_OUTPUT}"
  send_mail "🚨 SWS nightly — gh pr create failed" "Branch was pushed (${BRANCH}) but PR creation failed at $(ts), and no open PR for the branch exists server-side after ${PR_CREATE_ATTEMPTS} attempt(s).
...
echo "[nightly] PR ready: ${PR_URL}"
# ── end PR create + reconcile ───────────────────────────────────────────────
...
# Server-side truth for "did the merge actually land". Needed because a 502 from
# api.github.com can arrive after the merge mutation committed — see the PR
# create + reconcile block above. Never infer merge success from a gh rc alone.
pr_is_merged() {
  local pr_url="$1"
  [ "$(gh pr view "${pr_url}" --json state --jq '.state' 2>/dev/null)" = "MERGED" ]
}
...
if [ "${AUTO_MERGE}" = "1" ]; then
  ...
  if ! gh pr merge "${PR_URL}" --squash --auto 2>&1 | sed 's/^/[gh] /'; then
    ...
    gh pr merge "${PR_URL}" --squash 2>&1 | sed 's/^/[gh] /' || {
      # Same 502-after-commit hazard as gh pr create: a non-zero rc here can
      # still mean the squash landed. Confirm against server state before
      # paging a human and abandoning the deploy-trigger path below.
      if pr_is_merged "${PR_URL}"; then
        echo "[nightly] gh pr merge reported failure but the PR is MERGED server-side — continuing"
      else
...
# The SWS scrape can ship perfectly while the independent data-only ship fails.
# That still means data did not reach prod, so it must not look like a clean run.
# Exit codes 3-8 are already taken; 9 == data-only ship failed.
if [ "${DATA_SHIP_FAILED}" -eq 1 ]; then
  echo "[nightly] SWS shipped, but the data-only ship FAILED — exiting 9"
  exit 9
fi
exit 0
