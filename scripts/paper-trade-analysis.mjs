#!/usr/bin/env node
/**
 * Paper Trading Simulation & Report Generator
 *
 * 12-month backtest using the platform's ACTUAL production scoring engine.
 * Fetches 18 months of OHLCV data (Oct 2024 - Apr 2026) for all Nifty 100
 * stocks + Nifty 50 index, runs monthly scans (Apr 2025 - Mar 2026), tracks
 * trades with SL/target exit logic, runs sensitivity analysis, and generates
 * a comprehensive markdown report.
 *
 * Usage:
 *   node scripts/paper-trade-analysis.mjs
 */
...
const REPORT_PATH = path.join(REPORT_DIR, "paper-trading-report.md");

// ==================== CONFIGURATION ====================

const HISTORY_START = "2024-10-01";
...
async function runSimulation() {
  ...
  const fundHistory = loadFundamentalsHistory();
  if (!fundHistory) {
  ...
  // Step 4: Run 12 monthly scans
  console.log("Running 12 monthly scans...\n");
  const allTrades = [];
...
function runSensitivityAnalysis(stocks, histories) {
  const weightSplits = [
    { techWeight: 0.30, fundWeight: 0.70, label: "30/70" },
  ...
  const slMultipliers = [3, 4, 5];
  const scoreThresholds = [55, 60, 65, 70];
...
function generateReport(allTrades, monthlyResults, niftyBars, histories, stocks, sensitivityResults) {
  ...
  md += `**Period:** Apr 2025 - Mar 2026 (12 monthly scans)  \n`;
  ...
  md += "1. **Historical fundamentals approximation:** The simulation uses the current fundamentals.json snapshot for all 12 monthly scans. In reality, fundamental scores would have changed over time as earnings were reported. This introduces look-ahead bias for fundamental-driven categories.\n";
