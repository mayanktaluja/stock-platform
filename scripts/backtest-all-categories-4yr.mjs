#!/usr/bin/env node
/**
 * ALL CATEGORIES -- 4-Year Honest Paper Trading Simulation
 * =========================================================
 *
 * Head-to-head comparison of the four user-facing pick categories with
 * independent capital pools (Rs 2,00,000 each = Rs 8,00,000 total).
 * Intraday is excluded — the user did not ask for it and the 3yr report
 * showed XIRR maths breaks when intraday capital approaches zero.
 *
 * Categories:
 *   1. Deep Value Picks        (verdict = DEEP_VALUE)
 *   2. Quality Growth Picks    (verdict = QUALITY_GROWTH)
 *   3. Best Stocks to Buy Now  (combined tech+fund >= 65, dual-lane)
 *   4. Mid-Term Investment     (midTermScore >= 58, 4-week hold)
 *
 * 48 monthly scans: Apr 2022 - Mar 2026
 * History: Apr 2021 - Apr 2026 (12 months warmup so 200DMA is valid
 * from the very first scan).
 *
 * Honesty caveats (REPORTED IN OUTPUT):
 *   - Look-ahead bias: fundamentals are current (Apr 2026) snapshot.
 *   - Survivorship bias: universe = today's Nifty 100.
 *   - No friction in this script (matches legacy 3yr report). Subtract
 *     ~50-100 bps from each XIRR for a real-money estimate.
 *
 * Usage: node scripts/backtest-all-categories-4yr.mjs
 */
...
const YEARS = CLI_ARGS.years ? parseInt(CLI_ARGS.years, 10) : 4;
if (![1, 2, 3, 4, 5].includes(YEARS)) {
  console.error("FATAL: --years must be one of 1, 2, 3, 4, 5");
  process.exit(1);
}

// --topN can be a single value or CSV list, e.g. --topN=1,3,5,10. Default 10.
const TOP_N_LIST = (CLI_ARGS.topN ? String(CLI_ARGS.topN) : "10")
  .split(",").map(s => parseInt(s.trim(), 10)).filter(n => n >= 1 && n <= 20);
if (TOP_N_LIST.length === 0) {
  console.error("FATAL: --topN must be 1+ integers in [1,20]");
  process.exit(1);
}

// --universe = nifty100 (default) | nifty500
const UNIVERSE = CLI_ARGS.universe || "nifty100";
if (!["nifty100", "nifty500"].includes(UNIVERSE)) {
  console.error("FATAL: --universe must be nifty100 or nifty500");
  process.exit(1);
}

// --techWeight = 0..1 (Buy Now scanner only). Fund weight = 1 - tech.
const TECH_WEIGHT = CLI_ARGS.techWeight != null ? parseFloat(CLI_ARGS.techWeight) : 0.5;
if (!Number.isFinite(TECH_WEIGHT) || TECH_WEIGHT < 0 || TECH_WEIGHT > 1) {
  console.error("FATAL: --techWeight must be in [0,1]");
  process.exit(1);
}
const FUND_WEIGHT = 1 - TECH_WEIGHT;

const yf = new YahooFinance({ suppressNotices: ["yahooSurvey"] });

// ==================== CONFIGURATION ====================

// Scans run from (2026 - YEARS) Apr → 2026 Mar. Warmup = 1y before scan start.
// e.g. YEARS=3 → scan Apr 2023 - Mar 2026, history Apr 2022 - Apr 2026.
const SCAN_END_YEAR = 2026; // last scan in Mar of this year
const SCAN_START_YEAR = SCAN_END_YEAR - YEARS;
const HISTORY_START = `${SCAN_START_YEAR - 1}-04-01`;
const HISTORY_END   = "2026-04-25";
...
function generateScanDates() {
  ...
  // Apr SCAN_START_YEAR through Mar SCAN_END_YEAR = (YEARS * 12) months
  for (let y = SCAN_START_YEAR; y <= SCAN_END_YEAR; y++) {
    const sm = (y === SCAN_START_YEAR) ? 3 : 0;  // Apr in start year
    const em = (y === SCAN_END_YEAR)   ? 2 : 11; // Mar in end year
    for (let m = sm; m <= em; m++) {
...
function scanBuyNow(stocks, histories, scanDateStr) {
  ...
  for (const stock of stocks) {
    ...
    const combined = techScore * TECH_WEIGHT + fundScore * FUND_WEIGHT;
...
async function runSimulation() {
  ...
  console.log(`  ALL CATEGORIES -- ${YEARS}-Year Honest Paper Trading Simulation`);
  console.log(`  Apr ${SCAN_START_YEAR} - Mar ${SCAN_END_YEAR} (${SCAN_DATES.length} monthly scans)`);
  console.log(`  Universe: ${UNIVERSE} | Top-N values: [${TOP_N_LIST.join(", ")}]`);
  console.log(`  Capital: Rs 2,00,000 per category (Rs ${(STARTING_CAPITAL * 4 / 1e5).toFixed(0)},00,000 total)`);
  console.log("=".repeat(70));
  console.log();
  ...
  console.log(`Universe: ${stocks.length} ${UNIVERSE === "nifty500" ? "Nifty 500" : "Nifty 100"} stocks\n`);

  // Step 3: Fetch all historical data (ONCE — reused across all topN runs)
  ...
  // Loop over each topN value, running a full simulation for each.
  for (const TOP_N of TOP_N_LIST) {
    console.log("\n" + "=".repeat(70));
    console.log(`  RUNNING SIMULATION WITH TOP-${TOP_N} PICKS PER CATEGORY`);
    console.log("=".repeat(70) + "\n");
    await runOneTopN(TOP_N, stocks, histories, niftyBars);
  }
}

async function runOneTopN(TOP_N, stocks, histories, niftyBars) {
  // Per-topN report path (and override the module-level REPORT_PATH used by generateReport).
  const REPORT_PATH_LOCAL = path.join(REPORT_DIR, `all-categories-${YEARS}yr-${UNIVERSE}-top${TOP_N}-report.md`);

  // Step 4: Run monthly scans for all 5 categories with independent capital pools
  console.log(`Running ${SCAN_DATES.length} monthly scans for ALL ${CATEGORIES.length} categories...\n`);
  ...
  for (const scan of SCAN_DATES) {
    ...
    // ---- Category 1: Deep Value ----
    {
      ...
      const picks = scanDeepValue(stocks, histories, scan.date).slice(0, TOP_N);
    ...
    // ---- Category 2: Quality Growth ----
    {
      ...
      cat.monthlyCapital.push({ label: scan.label, capital: cat.capital });
      const picks = scanQualityGrowth(stocks, histories, scan.date).slice(0, TOP_N);
      if (picks.length > 0 && cat.capital > 0) {
        const perPick = cat.capital / picks.length;
        let returned = 0;
        for (const p of picks) {
    ...
    // ---- Category 3: Best Stocks to Buy Now ----
    {
      ...
      cat.monthlyCapital.push({ label: scan.label, capital: cat.capital });
      const picks = scanBuyNow(stocks, histories, scan.date).slice(0, TOP_N);
      if (picks.length > 0 && cat.capital > 0) {
        const perPick = cat.capital / picks.length;
        let returned = 0;
        for (const p of picks) {
    ...
    // ---- Category 4: Mid-Term ----
    {
      ...
      cat.monthlyCapital.push({ label: scan.label, capital: cat.capital });
      const picks = scanMidTerm(stocks, histories, scan.date).slice(0, TOP_N);
      if (picks.length > 0 && cat.capital > 0) {
        const perPick = cat.capital / picks.length;
        let returned = 0;
        for (const p of picks) {
  ...
  console.log("\nGenerating report...");
  generateReport(catState, categoryMetrics, catStockPicks, niftyBars, niftyXIRRVal, niftyReturn, niftyStartPrice, niftyEndPrice, REPORT_PATH_LOCAL, TOP_N);
  console.log(`Report saved to: ${REPORT_PATH_LOCAL}`);

  if (CLI_ARGS["dump-picks"]) {
    const picksByMonth = {};
    for (const cat of CATEGORIES) {
      for (const t of catState[cat.key].trades) {
        const key = t.scanDateRaw;
        if (!picksByMonth[key]) picksByMonth[key] = { date: t.scanDateRaw, label: t.scanDate, picks: {} };
        if (!picksByMonth[key].picks[cat.key]) picksByMonth[key].picks[cat.key] = [];
        picksByMonth[key].picks[cat.key].push({
          ...
          exitDate: t.exitDate, reason: t.reason,
          returnPct: Math.round(t.returnPct * 100) / 100,
          score: t.fundScore ?? t.combinedScore ?? t.midTermScore ?? null,
          verdict: t.fundVerdict ?? null,
        });
      }
    }
    const out = Object.values(picksByMonth).sort((a, b) => a.date.localeCompare(b.date));
    const dumpPath = path.join(REPORT_DIR, `picks-by-month-${YEARS}yr-${UNIVERSE}-top${TOP_N}.json`);
    writeFileSync(dumpPath, JSON.stringify(out, null, 2));
    console.log(`Picks dump saved to: ${dumpPath}`);
  }
...
function generateReport(catState, metrics, catStockPicks, niftyBars, niftyXIRR, niftyReturn, niftyStart, niftyEnd, reportPathOverride, topN) {
  if (!existsSync(REPORT_DIR)) mkdirSync(REPORT_DIR, { recursive: true });

  const reportPath = reportPathOverride || REPORT_PATH;
  let md = "";

  // ============================================================
  // HEADER
  // ============================================================
  md += `# All Categories -- ${YEARS}-Year Honest Paper Trading Report (${UNIVERSE.toUpperCase()}, top-${topN})\n\n`;
  md += `**Generated:** ${new Date().toISOString().slice(0, 10)}  \n`;
  md += `**Scan Period:** Apr ${SCAN_START_YEAR} - Mar ${SCAN_END_YEAR} (${SCAN_DATES.length} monthly scans)  \n`;
  md += `**History:** Apr ${SCAN_START_YEAR - 1} - Apr ${SCAN_END_YEAR} (${YEARS + 1} years OHLCV with 1y warmup)  \n`;
  md += `**Capital:** Rs 2,00,000 per category (Rs ${fmtRupee(STARTING_CAPITAL * CATEGORIES.length)} total), compounding monthly  \n`;
  md += `**Universe:** Nifty 100 stocks (Nifty 100 = top 100 NSE stocks by free-float market cap)  \n`;
  md += `**Friction:** None applied in this script — subtract ~50-100 bps from each XIRR for a real-money estimate  \n`;
  ...
  for (const cat of CATEGORIES) {
    ...
    for (let yi = 0; yi < YEARS; yi++) {
      const startYr = SCAN_START_YEAR + yi;
      const endYr = startYr + 1;
      yearRanges.push({
        label: `Year ${yi + 1} (Apr ${startYr} - Mar ${endYr})`,
        start: yi * 12,
        end: yi * 12 + 11,
      });
    }
  ...
  md += `If you invested Rs 2,00,000 in EACH category (Rs ${fmtRupee(STARTING_CAPITAL * CATEGORIES.length)} total):\n\n`;

  const totalInvested = STARTING_CAPITAL * CATEGORIES.length;
  const totalFinal = CATEGORIES.reduce((s, c) => s + catState[c.key].capital, 0);
  const totalReturn = ((totalFinal - totalInvested) / totalInvested) * 100;
  const niftyEquiv = totalInvested * (1 + niftyReturn / 100);
  ...
  md += `| Nifty Equivalent (Rs ${fmtLakh(totalInvested)} buy & hold) | Rs ${fmtRupee(niftyEquiv)} (${fmtLakh(niftyEquiv)}) |\n`;
  ...
  md += `Do categories pick the same stocks? (unique symbols picked across all ${SCAN_DATES.length} months)\n\n`;
  ...
  for (const cat of CATEGORIES) {
    ...
    for (let mi = 0; mi < SCAN_DATES.length; mi++) {
  ...
  md += `**Diversification benefit:** In ${diversificationWins}/${SCAN_DATES.length} months, at least one category was positive while another was negative. This natural hedging reduces portfolio volatility.\n\n`;
  md += "---\n\n";
  ...
  md += `Based on ${YEARS} years of data across ${CATEGORIES.length} categories:\n\n`;
  ...
  md += "4. **Compounding optimization:** Instead of equal split, dynamically allocate more capital to categories with recent momentum\n";
  md += "5. **Transaction cost modeling:** Add 0.05-0.10% round-trip costs per trade for realistic P&L\n";
  md += "6. **Drawdown limits:** Add a per-category drawdown circuit breaker (e.g., pause if capital drops 30% from peak)\n";
  md += "7. **Point-in-time fundamentals:** Use pointInTimeFundamentals.js (already in repo) so 2022-2023 scans don't see 2025 EPS prints — eliminates the look-ahead disclaimer.\n\n";
  ...
  md += `1. **Historical fundamentals approximation:** Uses the current (Apr 2026) fundamentals.json snapshot for all ${SCAN_DATES.length} scans. In reality, fundamental scores change quarterly as earnings are reported. This introduces look-ahead bias -- results for 2022-2023 are most affected and should be interpreted with caution. The platform has pointInTimeFundamentals.js available but this script does not yet wire it in.\n`;
  ...
  md += "7. **Compounding risk:** Each category's capital fully compounds monthly. A bad month can significantly reduce future capital, amplifying drawdowns.\n\n";
  md += "---\n\n";
