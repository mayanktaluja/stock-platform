#!/usr/bin/env node
/**
 * 8-Horizon Paper Trading Simulation (Post P0-P6 Improvements)
 * ══════════════════════════════════════════════════════════════════
 *
 * Runs 8 horizon tests (1yr through 8yr) with the IMPROVED platform:
 *   P0: No macro mood filter on Buy Now / Fundamental picks
 *   P1: SL widened to ATR × 4
 *   P2: Target tightened to ATR × 5
 *   P4: Sector exclusion (Banking, Tourism, Cement, Chemicals)
 *   P5: QUALITY_GROWTH overweighted (+5 sort bonus)
 *   P6: Trailing stop info computed (for UI display)
 *
 * Usage:
 *   node scripts/backtest-8yr.mjs
 */
...
const REPORT_PATH = path.join(REPORT_DIR, "8-horizon-report.md");

// ==================== CONFIGURATION ====================

// 8-year fetch: need 6mo warmup before Apr 2018
const HISTORY_START = "2017-10-01";
...
const HORIZONS = [
  ...
  { label: "6 Years", startYear: 2020, startMonth: 3, endYear: 2026, endMonth: 2 },
  { label: "7 Years", startYear: 2019, startMonth: 3, endYear: 2026, endMonth: 2 },
  { label: "8 Years", startYear: 2018, startMonth: 3, endYear: 2026, endMonth: 2 },
];
...
function trackTrade(forwardBars, entryPrice, target, stopLoss, maxExitDateStr) {
  ...
  // Fix 1: 2-close SL confirmation — count consecutive closes below SL
  let slCloseCount = 0;
  // Fix 4: Trailing stop — track highest close, compute dynamic SL
  let highestClose = entryPrice;
  const atrEstimate = stopLoss ? (entryPrice - stopLoss) / 4 : 0; // reverse-engineer ATR from SL
  const trailDist = atrEstimate * 3;
  ...
  for (const bar of forwardBars) {
    ...
    const effectiveSL = stopLoss ? Math.max(stopLoss, trailingLevel) : trailingLevel;

    // Fix 1: Check SL with 2-close confirmation
    if (effectiveSL && bar.close < effectiveSL) {
      ...
      if (slCloseCount >= 2) {
        return {
          ...
          exitReason: "SL_CONFIRMED",
          returnPct: ((bar.close - entryPrice) / entryPrice) * 100,
        };
      }
    } else {
      slCloseCount = 0; // reset if close is above SL
    }

    // Fix 4: Check trailing stop trigger (single close, since it's already confirmed by trailing)
    if (trailDist > 0 && bar.close < trailingLevel && trailingLevel > stopLoss) {
      // Trailing stop triggered (only when it's moved above the initial SL)
      return {
        exitPrice: parseFloat(bar.close.toFixed(2)),
        exitDate: toDateStr(bar.date),
...
function runScan(stocks, histories, scanDateStr, activePositions, maxPicks, scanStats) {
  ...
  // Pass 1: priceMap for point-in-time sector P/E medians
  ...
  for (const stock of stocks) {
    ...
    // P1+P2: SL 4× ATR, Target 5× ATR
    const midTermSL = atr ? entryPrice - atr * 4 : midTerm.stopLoss;
    const midTermTarget = atr ? entryPrice + atr * 5 : midTerm.target;

    // P4: Sector exclusion
    const EXCLUDED_SECTORS = ["Banking", "Tourism", "Cement", "Chemicals"];
    const sectorExcluded = EXCLUDED_SECTORS.some(ex =>
      (stock.sector || "").toLowerCase().includes(ex.toLowerCase())
    );

    // --- Category 1: Buy Now ---
    // P0: No macro filter applied (value picks benefit from buying dips)
    // P4: Excluded sectors filtered out
    // P5: QG gets +5 sort bonus (applied in sort below)
    // Unified 40/60 weighting to match server.js live + precompute scanner.
    if (fundScore != null && !sectorExcluded) {
  ...
  // P5: QUALITY_GROWTH gets +5 sort bonus
  buyNowCandidates.sort((a, b) => {
    const aBonus = a.fundVerdict === "QUALITY_GROWTH" ? 5 : 0;
    const bBonus = b.fundVerdict === "QUALITY_GROWTH" ? 5 : 0;
...
function runHorizon(horizon, stocks, histories, niftyBars, useFilter) {
  ...
  for (const scan of scanDates) {
    ...
    if (useFilter) {
      if (mood.picks === 0) {
        // STAY_OUT — skip this month entirely
        skippedMonths++;

        // Compute what Nifty did this month for validation
        const nextScan = scanDates[scanDates.indexOf(scan) + 1];
        let niftyMonthlyReturn = null;
        if (nextScan) {
          ...
          if (nStart.length > 0 && nEnd.length > 0) {
            ...
            niftyMonthlyReturn = ((ep - sp) / sp) * 100;
          }
        }
        ...
        monthlyResults.push({
          ...
          skipped: true,
          avgReturn: 0,
          niftyMonthly: niftyMonthlyReturn,
        });
        continue;
      }
      maxPicks = mood.picks;
