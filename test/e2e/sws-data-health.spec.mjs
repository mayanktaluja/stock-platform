...
test.describe("SWS data health", () => {
  ...
  test("US Gap Lab rows resolve to warning-compatible stock detail metadata", async ({ request }) => {
    const picksRes = await request.get("/api/us-picks");
    if (picksRes.status() === 404) return;
    expect(picksRes.status()).toBe(200);
    const picks = await picksRes.json();
    const gapRows = Array.isArray(picks?.sections?.snowflake_gap_lab)
      ? picks.sections.snowflake_gap_lab.slice(0, SAMPLE_PER_MARKET)
      : [];
    test.skip(gapRows.length === 0, "no US Snowflake Gap Lab rows in current fixture");
    ...
    for (const row of gapRows) {
      ...
      if (row?.canonical_score?.version !== "v4") {
        failures.push({ ticker: row.ticker, error: "canonical V4 missing on card" });
      }

      const detailRes = await request.get(`/api/us-stock/${encodeURIComponent(row.ticker)}`);
      if (detailRes.status() !== 200) {
        failures.push({ ticker: row.ticker, status: detailRes.status() });
        continue;
      }
      const detail = await detailRes.json();
      const ov = detail?.deep?.overview || {};
      if (!detail?.card?.snowflake_gap_lab) {
        failures.push({ ticker: row.ticker, status: 200, error: "missing card Gap Lab metadata" });
      }
      if (ov.snowflake_data_quality?.insufficient !== true) {
        failures.push({ ticker: row.ticker, status: 200, error: "missing insufficient-data metadata" });
      }
    ...
    expect(failures, `US Gap Lab detail warning contract drift: ${JSON.stringify(failures)}`).toEqual([]);
  });
