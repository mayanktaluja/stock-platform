// Chunked progressive render for the Avoid section.
//
// The Avoid bucket carries ~1206 stocks. Clicking "Show all (N)" used to
// build all N cards in one synchronous innerHTML assignment, blocking the
// main thread for 2–5 s and visibly freezing the UI. This spec is the
// regression gate for the fix: render only the first PICKS_EXPANDED_CHUNK_SIZE
// (100) cards inline + an IntersectionObserver sentinel, then stream the
// rest in as the user scrolls.
//
...
test.describe("SWS Picks · Avoid chunked progressive render", () => {
  test("clicking 'Show all (N)' on Avoid does not freeze the main thread", async ({ page }) => {
    await gotoApp(page, { tab: "picks" });
    await waitForPicksLoaded(page);

    // Read the Avoid count from the chip-nav. Self-skip if the bucket is
    ...
    // fail noisily.
    const avoidCountText = await page
      .locator('.sws-pick-chip[data-section-key="avoid"] .sws-pick-chip-count')
      .textContent({ timeout: 10_000 });
    const avoidCount = parseInt((avoidCountText || "").trim(), 10);
    test.skip(
      !Number.isFinite(avoidCount) || avoidCount < SKIP_BELOW,
      `Avoid count (${avoidCount}) below ${SKIP_BELOW} — chunked render path not exercised in this snapshot`
    );

    // Make sure the Avoid section is visible on screen so the "Show all"
    // button is clickable. The chip click expands + scrolls.
    await page.evaluate(() => window.jumpToPicksSection("avoid"));
    const showAllBtn = page
      .locator('.sws-pick-section[data-section-key="avoid"] .sws-pick-overflow-btn')
      .first();
    ...
    const avoidCardsAfterClick = await page
      .locator('.sws-pick-section[data-section-key="avoid"] .sws-pick-card')
      .count();
    expect(avoidCardsAfterClick).toBeGreaterThanOrEqual(CHUNK_SIZE);
    expect(avoidCardsAfterClick).toBeLessThan(avoidCount);
    await expect(
      page.locator('.sws-pick-chunk-sentinel[data-section-key="avoid"]')
    ...
    const maxIterations = Math.ceil(avoidCount / CHUNK_SIZE) + 3;
    for (let i = 0; i < maxIterations; i++) {
      const sentinelStillThere = await page
        .locator('.sws-pick-chunk-sentinel[data-section-key="avoid"]')
        .count();
    ...
    // Final card count in the Avoid section should equal the chip count.
    const finalCards = await page
      .locator('.sws-pick-section[data-section-key="avoid"] .sws-pick-card')
      .count();
    expect(finalCards).toBe(avoidCount);
  });
});
